...
def review(dir_path):
    ...
    for test_key in result["test_runs"].keys():
        for test_run_key in result["test_runs"][test_key].keys():
            ...
            if partitions not in best or throughput > best[partitions]["throughput"]:
                best[partitions] = {
    ...
    data.sort(key=lambda x:x["partitions"])
    return data
...
for item in data:
    ...
    threads_boundary = max(threads_boundary, item["partitions"])
    latency.write(f"{item['partitions']}\t{item['p50']}\n")
    throughput.write(f"{item['partitions']}\t{item['throughput']}\n")
