...
docker-compose -f docker/docker-compose3.2.yaml --project-directory . down --remove-orphans || true
docker-compose -f docker/docker-compose3.2.yaml --project-directory . rm -f || true
...
mkdir -p ./docker/bind_mounts/client2/mnt/vectorized/workloads/logs

chmod a+rw -R ./docker/bind_mounts

docker-compose -f docker/docker-compose3.2.yaml --project-directory . build --build-arg REDPANDA_DEB=$(basename $DEB_PATH) --build-arg USER_ID=$(id -u $(whoami))